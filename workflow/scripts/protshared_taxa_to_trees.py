...
# create a dict to store the genus assignments from all the markers to see if they disagree
# k=genome, v=[genus1, genus2...]
new_genus_assignments = dict()
        ...
        # it is not from the reference set
            ...
            if genome in shared_taxa.index:
                ...
                # shared value is close to 0.7
                    ...
                    print(f"{genome}: shared value below {snakemake.config.get('shared_genus_cutoff')}, but classified as {marker_taxa_genome} by {marker}.")
                ...
                if len(genus_leaves) > 1:
                    ...
                    for leaf in final_lca.iter_leaves():
                        if leaf.genus == "unknown":
                            ...
                            # add to the new assignments dictionary
                            if leaf.genome in new_genus_assignments:
                                new_genus_assignments[leaf.genome] += [genus]
                            else:
                                new_genus_assignments[leaf.genome] = [genus]



print("checking agreement between assignments across diferent markers")
non_agreement = list()
for genome, genera in new_genus_assignments.items():
    if len(list(set(genera))) > 1:
        print("Watch out!! Discarding", genome, genera)
        non_agreement.append(genome)


print(non_agreement)


# parse the trees again, this time knowing for which genomes the markers don't agree
markers_taxa = pd.read_csv(snakemake.input.taxa_markers[0], sep="\t", header=0, index_col=0)
shared_taxa = pd.read_csv(snakemake.input.taxa_shared[0], sep="\t", header=0, index_col=0)
shared_taxa = shared_taxa[shared_taxa["complete"] == "yes"]
genera = list(set([crass_taxonomy[genome]["genus"] for genome in crass_taxonomy]))
        ...
        if lcas:
            ...
            for lca in lcas:
                ...
                if len(genus_leaves) > 1:
                    ...
                    # iterate the final_lca while assigning taxonomy
                    for leaf in final_lca.iter_leaves():
                        if leaf.genus == "unknown" and leaf.genome not in non_agreement:
                            print(f"genus assigned to {leaf.genome}, {genus}")
                            markers_taxa.loc[leaf.genome, f"genus_{marker}"] = genus
                            # recalculate completeness, this time based on genus
                            genome_length = int(leaf.genome.split("_")[-1])
                            completenes = round((genome_length*100)/taxas_lengths[genus], 2)
