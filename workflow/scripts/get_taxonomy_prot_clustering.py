...
# ------------------------------------------------------------------------------------------
# read shared content matrix and iterate the genomes in the columns assessing their taxonomy
matrix_shared = pd.read_csv(snakemake.input.matrix_shared, sep="\t", header=0, index_col=0)

to_write = list()
# find out which genomes share >cutoff with any other genome
for qgenome in matrix_shared.columns[4:]: # exclude taxonomy ranks columns
    to_add = [qgenome]
...
# -------------------------------------------------------------
# Convert to df and write to file "shared_content_taxonomy.txt"
