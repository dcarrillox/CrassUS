...
# check if fastani file is empty
if os.path.getsize(snakemake.input.ani[0]) != 0:

    # read with pandas while sorting by cov
    df = pd.read_csv(snakemake.input.ani[0], sep="\t", header=None, names=["query", "ref", "ani", "cov", "total"]).sort_values("cov", ascending=False, ignore_index=True)
